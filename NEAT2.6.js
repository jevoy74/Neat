...
function PopupContinue()
{
   ...
   var OkButton	       = 0;
   var Timeout         = 4;
   var Title = "Nortel Enterprise Audit Tool Ver 2.6                       - Jason Evoy";
...
function GetVoiceServices()
{
	...
	TotalMessage += (shell.RegRead( regkey7 ));
...
function GetSybaseVer()
{
	...
	TotalMessage += (shell.RegRead( regkey1 ));
	TotalMessage += ( cr + "ODBC Blue ServerName: " );
	TotalMessage += (shell.RegRead( regkey2 ));
	return( TotalMessage + cr );
    }
    catch( e )
...
function GetSMI()
{
    ...
    {
        var cr = "\r\n";
	var TotalMessage = "Client Version: ";
	var shell = new ActiveXObject( "WScript.Shell" );
       	var regkey2 = "HKLM\\Software\\Nortel\\SMI Services\\SCCS\\Service Build";
	var regkey3 = "HKLM\\Software\\SYBASE\\Open Client\\CurrentVersion";
  	TotalMessage += (shell.RegRead( regkey2 ) + cr );
	TotalMessage += "Sybase Open Client Ver: ";
	TotalMessage += (shell.RegRead( regkey3 ) + cr );
	return( TotalMessage );
    }
    catch( e )
