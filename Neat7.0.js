...
function PopupContinue()
{
   ...
   var Title = "Avaya Enterprise Audit Tool Ver 7.0                  - Jason Evoy";
   ...
   var MessageLine2 = "\n\nFor computers that are running older operating systems, please use Neat4.3.js for better results";
...
function MakeFile()
   ...
   var UNICODE = -1, ASCII = 0, SystemDefault = -2;
   ...
   os = os.OpenAsTextStream( forAppending, -2);
   var LineBreak = "+-------------------------------------------------------------------------+";
   os.write(LineBreak + cr);
   os.write("¦Avaya Enterprise Audit Tool v7.0 Results - ");
   os.write(new Date());
   os.write(cr);
...
Sleep();
   ...
   os = shell.GetFile( GetComputerName() + ".txt" );
   os = os.OpenAsTextStream( forAppending, -2);
...
function PSGetNetTCPSetting()
{
   var Wshell = new ActiveXObject("Shell.Application");
   var command = "/c powershell get-NetTCPSetting >>" + FindDirectory() + GetComputerName()+ ".txt";
   Wshell.ShellExecute ("cmd.exe", command, "", "runas", 0);
   return;
}

function PSHostsHeading()
{
   var Wshell = new ActiveXObject("Shell.Application");
   var command  = "/c powershell Get-Content $env:SystemRoot\\System32\\Drivers\\etc\\hosts >>" + FindDirectory() + GetComputerName()+ ".txt";
   var Heading ="/c powershell Write-Host \"+-------------------------------------------------------------------------+`n: Hosts File`n+-------------------------------------------------------------------------+`n\" >>" + FindDirectory() + GetComputerName()+ ".txt";
   Wshell.ShellExecute ("cmd.exe", Heading, "", "runas", 0);
   return;
}

function PSGetHostsFile()
{
   var Wshell = new ActiveXObject("Shell.Application");
   var command = "/c powershell Get-Content $env:SystemRoot\\System32\\Drivers\\etc\\hosts >>" + FindDirectory() + GetComputerName()+ ".txt";
   Wshell.ShellExecute ("cmd.exe", command, "", "runas", 0);
...
{
   var Cut = WScript.ScriptFullName.length - WScript.ScriptName.length;
   var Directory = WScript.ScriptFullName.substring(0, Cut);
   return (Directory);
}
...
{
   var Wshell = new ActiveXObject("Shell.Application");
   var command = "/c powershell -noexit -executionpolicy bypass -nologo -file Neat7.0.ps1 >>" + FindDirectory() + GetComputerName()+ ".txt";
   Wshell.ShellExecute ("cmd.exe", command, "", "runas", 0);
   return;
}