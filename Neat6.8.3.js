 ...
 *                Will run on any PC but specifically designed for AACC 6.0 and above
...
function PopupContinue()
{
   ...
   var Title = "Avaya Enterprise Audit Tool Ver 6.8.3                  - Jason Evoy";
...
function MakeFile()
   ...
   os = os.OpenAsTextStream( forAppending, -1);
   var LineBreak = "+-------------------------------------------------------------------------+";
   os.write(LineBreak + cr);
   os.write("¦Avaya Enterprise Audit Tool v6.8.3 Results - ");
   os.write(new Date());
   os.write(cr);
   ...
   os.write(GetAACC7PEPS());
   os.write(GetCCCC7PEPS());
   os.write(GetCCMA7PEPS());
   os.write(GetCCLM7PEPS());
   os.write(GetCCMM7PEPS());
   os.write(GetCCMSU7PEPS());
   os.write(GetCCT7PEPS());
   os.write(GetCCWS7PEPS());
   ...
   os.write(LineBreak + cr);
   os.write("¦***** END OF LOG ***** Completed: " + (new Date()) + cr);
   os.write(LineBreak + cr);
   os.Close();
   }
...
function PopupError()
  {
    var Title = "ERROR: This is a modified version of Neat & has been removed for your safety";
    var shell = WScript.CreateObject("WScript.Shell");
...
function GetSIPInfo()
{
	...
	var regkey4 = "HKLM\\Software\\Wow6432Node\\Nortel\\Setup\\Switch\\ContactCenterName";
	var regkey5 = "HKLM\\Software\\Wow6432Node\\Nortel\\Setup\\Switch\\WSClientPort";
	var regkey6 = "HKLM\\Software\\Wow6432Node\\Nortel\\Setup\\Switch\\WSServerPort";
	var regkey7 = "HKLM\\Software\\Wow6432Node\\Nortel\\Setup\\Switch\\AuraVersion";
	var regkey8 = "HKLM\\Software\\Wow6432Node\\Nortel\\Setup\\Switch\\SIPProxy";
	var regkey9 = "HKLM\\Software\\Wow6432Node\\Nortel\\Setup\\Switch\\CTIProxyDetails";

	var SIPType = shell.RegRead( regkey1 );
	var SwitchSubType = "";
	if (SIPType == 5100) { SwitchSubType += "MCS 5100 (" + SIPType +")" };
	if (SIPType == 5101) { SwitchSubType += "MCS 5200 (" + SIPType +")" };
	if (SIPType == 5102) { SwitchSubType += "OCS (" + SIPType +")" };
	if (SIPType == 5103) { SwitchSubType += "OCS Standalone (" + SIPType +")" };
	if (SIPType == 5104) { SwitchSubType += "Cisco (" + SIPType +")" };
	if (SIPType == 5105) { SwitchSubType += "Native SIP (" + SIPType +")" };
	if (SIPType == 5107) { SwitchSubType += "Aura 6.x (" + SIPType +")" };
	if (SIPType == 5111) { SwitchSubType += "IPOffice (" + SIPType +")" };
	
	var TotalMessage = "\r\nLocal SIP Provider: \r\n";
	TotalMessage += ("SIP Server Type: " + SwitchSubType + cr);
	TotalMessage += ("SIP Server Version: " + shell.RegRead( regkey7 ) + cr);
	TotalMessage += ("Media Server Locale: " + shell.RegRead( regkey2 ) + cr);
	TotalMessage += ("Contact Center Name: " + shell.RegRead( regkey4 ) + cr);
	TotalMessage += ("Domain Name: " + shell.RegRead( regkey3 ) + cr);
	TotalMessage += ("Web Service Server Port: " + shell.RegRead( regkey6 ) + cr);
	TotalMessage += ("Web Service Client Port: " + shell.RegRead( regkey5 ) + cr);
	TotalMessage += "\r\nSIP Network Settings: \r\n";
	TotalMessage += ("Voice Proxy Server: " + shell.RegRead( regkey8 ) + cr);
	TotalMessage += ("CTI Proxy Server:  " + shell.RegRead( regkey9 ) + cr);
...
function GetAACC7PEPS()
{
	...
	TotalMessage = LineBreak + cr;
        TotalMessage +="¦Installed Patches as of: " + new Date() + "                                  ¦" + cr;
        TotalMessage += LineBreak + cr + cr;
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\CCMS"
...
function GetCCCC7PEPS()
{
	...
	HKCC = 0x80000005;
	TotalMessage = "",cr = "\r\n", TAB = "\t\t";
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\CommonComponents"
...
function GetCCMA7PEPS()
{
	...
	HKCC = 0x80000005;
	TotalMessage = "",cr = "\r\n", TAB = "\t\t";
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\CCMA"
...
function GetCCLM7PEPS()
{
	...
	HKCC = 0x80000005;
	TotalMessage = "",cr = "\r\n", TAB = "\t\t";
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\LM"
...
function GetCCMM7PEPS()
{
	...
	HKCC = 0x80000005;
	TotalMessage = "", cr = "\r\n", TAB = "\t\t";
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\CCMM"
...
function GetCCMSU7PEPS()
{
	...
	HKCC = 0x80000005;
	TotalMessage = "",cr = "\r\n", TAB = "\t\t";
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\CCMSU"
...
function GetCCT7PEPS()
{
	...
	HKCC = 0x80000005;
	TotalMessage = "",cr = "\r\n", TAB = "\t\t";
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\CCT"
...
function GetCCWS7PEPS()
{
	...
	HKCC = 0x80000005;
        TotalMessage = "",cr = "\r\n", TAB = "\t\t";
	RegistryPath = "SOFTWARE\\Wow6432Node\\Avaya\\Contact Center\\Product Updates\\CCWS"
...
try
   ...
   for (y = 0; y <= bNames.length -1; y++)
   {
      if (aTypes[y] == "1")
         {
         subkeyval = bNames[y];
            if (subkeyval == "Date" || subkeyval == "Time" || subkeyval == "LogonUser" || subkeyval == "BuildID")
            {
   	    oMethod = oReg.Methods_.Item("GetStringValue");
            oInParam = oMethod.InParameters.SpawnInstance_();
            oInParam.hDefKey = HKLM;
